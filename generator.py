...
class UNetGenerator:
    ...
    def __call__(self,input):
        ...
        with tf.variable_scope(self.name):        
            ...
            #print(input.shape)
            conv1 = conv_layer(input, input.get_shape()[3], 3, 64)
            #print(conv1.shape)
            conv2 = conv_layer(conv1, conv1.get_shape()[3], 3, 64)
            #print(conv2.shape)
            pool2 = pool_layer(conv2)
            #print(pool2.shape)
            conv3 = conv_layer(pool2, pool2.get_shape()[3], 3, 128)
            #print(conv3.shape)
            conv4 = conv_layer(conv3, conv3.get_shape()[3], 3, 128)
            #print(conv4.shape)
            pool4 = pool_layer(conv4)
            #print(pool4.shape)
            conv5 = conv_layer(pool4, pool4.get_shape()[3], 3, 256)
            #print(conv5.shape)
            conv6 = conv_layer(conv5, conv5.get_shape()[3], 3, 256)
            #print(conv6.shape)
            pool6 = pool_layer(conv6)
            #print(pool6.shape)
            conv7 = conv_layer(pool6, pool6.get_shape()[3], 3, 512)
            #print(conv7.shape)
            conv8 = conv_layer(conv7, conv7.get_shape()[3], 3, 512)
            #print(conv8.shape)
            pool8 = pool_layer(conv8)
            #print(pool8.shape)

            conv9 = conv_layer(pool8, pool8.get_shape()[3], 3, 1024)
            #print(conv9.shape)
            conv10 = conv_layer(conv9, conv9.get_shape()[3], 3, 1024)
            #print(conv10.shape)

            conv11 = un_conv(conv10, conv10.get_shape()[3], 2, 512, self.image_size // 8, train)
            #print(conv11.shape)
            merge11 = tf.concat(values=[conv8, conv11], axis = -1)
            #print(merge11.shape)

            conv12 = conv_layer(merge11, merge11.get_shape()[3], 3, 512)
            #print(conv12.shape)
            conv13 = conv_layer(conv12, conv12.get_shape()[3], 3, 512)
            #print(conv13.shape)

            conv14 = un_conv(conv13, conv13.get_shape()[3], 2, 256, self.image_size // 4, train)
            #print(conv14.shape)
            merge14 = tf.concat([conv6, conv14], axis=-1)
            #print(merge14.shape)

            conv15 = conv_layer(merge14, merge14.get_shape()[3], 3, 256)
            #print(conv15.shape)
            conv16 = conv_layer(conv15, conv15.get_shape()[3], 3, 256)
            #print(conv16.shape)

            conv17 = un_conv(conv16, conv16.get_shape()[3], 2, 128, self.image_size // 2, train)
            #print(conv17.shape)
            merge17 = tf.concat([conv17, conv4], axis=-1)
            #print(merge17.shape)

            conv18 = conv_layer(merge17, merge17.get_shape()[3], 3, 128)
            #print(conv18.shape)
            conv19 = conv_layer(conv18, conv18.get_shape()[3], 3, 128)
            #print(conv19.shape)

            conv20 = un_conv(conv19, conv19.get_shape()[3], 2, 64, self.image_size, train)
            #print("UP",conv20.shape)
            merge20 = tf.concat([conv20, conv2], axis=-1)
            #print(merge20.shape)

            conv21 = conv_layer(merge20, merge20.get_shape()[3], 3, 64)
            #print(conv21.shape)
            conv22 = conv_layer(conv21, conv21.get_shape()[3], 3, 64)
            #print(conv22.shape)
